...
BUCKET = 'protolith-app-model'
MODEL_KEY = 'Model50_15_full_2020-52-03-13-52.z'
...
def get_model():
    """Downloads model served directly from an AWS bucket
    """
    with BytesIO() as data:
        S3.Bucket(BUCKET).download_fileobj(MODEL_KEY, data)
...
def single_predict():
    ...
    print(output)
    return render_template('results.html', prediction = f'''<div class="alert alert-success">
...
def batch_predict():
    ...
    if file_upload and allowed_file(file_upload.filename):
        ...
        return render_template('results.html', table_error= f'''<div class="alert alert-warning">
    ...
    return render_template('results.html',download=download, tables=out_df.to_html(classes=['data','table-striped'], header=True, index=False,border=None))
